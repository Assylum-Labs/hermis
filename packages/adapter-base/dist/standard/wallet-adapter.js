...
export class StandardWalletAdapter {
                ...
                if (events && typeof events.on === 'function') {
                    this._removeAccountChangeListener = events.on('change', (event) => {
                        try {
                            ...
                            // Find Solana account
                            const solanaAccount = event.accounts.find((account) => account && account.features &&
    ...
    async connect() {
        try {
            ...
            // Find Solana account
            const solanaAccount = connectResult.accounts.find((account) => account && account.features &&
                Array.isArray(account.features) &&
                account.features.includes('solana:publicKey'));
            if (!solanaAccount) {
            ...
            if (SolanaSignAndSendTransactionMethod in this._wallet.features) {
                ...
                try {
                    ...
                    if (typeof window !== 'undefined') {
                        signatureBase64 = btoa(String.fromCharCode.apply(null, Array.from(new Uint8Array(result.signature))));
                    }
                    else {
                        // Node.js environment
