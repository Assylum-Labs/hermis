...
jest.mock('@hermis/solana-headless-core', () => {
    ...
    class Transaction {
        feePayer: any = null;
        serialize() { return new Uint8Array([1, 2, 3, 4]); }
    }
    class VersionedTransaction {
        static deserialize = jest.fn().mockReturnValue({
...
describe('StandardWalletAdapter', () => {
        ...
        test('should throw if no Solana accounts found', async () => {
            mockWalletFeatures[StandardConnectMethod].connect.mockResolvedValueOnce({
                accounts: [
                    {
                        address: 'mockAddress',
                        publicKey: new Uint8Array([1, 2, 3, 4]), features: ['ethereum:publicKey']
                    }
                ]
            });
    ...
    describe('transaction operations', () => {
        test('should sign transaction', async () => {
            ...
            // Instead of using Uint8Array directly, create a proper mock transaction object
            ...
            expect(mockWalletFeatures[SolanaSignTransactionMethod].signTransaction)
                .toHaveBeenCalledWith({ transaction: expect.any(Uint8Array) });
        });
