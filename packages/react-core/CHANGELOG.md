...
- c6f06b9: - @hermis/react-core: Network configuration changes - Introduced new TWalletAdapterNetwork enum type
  with standardized network values (mainnet-beta,
  devnet, testnet, localnet) - HermisProvider: endpoint and network props are
  now required (previously optional with defaults) - ContextProvider: network prop is now required
  and uses TWalletAdapterNetwork type - ConnectionProvider: Updated to use
  TWalletAdapterNetwork type instead of
  WalletAdapterNetwork - useConnection hook: network in
